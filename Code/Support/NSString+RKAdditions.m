...
- (NSString *)stringByAppendingQueryParameters:(NSDictionary *)queryParameters {
    ...
    NSMutableDictionary *dic = [[NSMutableDictionary alloc] initWithDictionary:queryParameters];
    NSString *hash;
    for (int i=0; i<[dic count]; i++) {
        if ([dic objectForKey:@"hash"]) {
            hash = [dic objectForKey:@"hash"];
            [dic removeObjectForKey:@"hash"];
            return [NSString stringWithFormat:@"%@?hash=%@&%@", self, hash, [queryParameters stringWithURLEncodedEntries]];
        }
    }
